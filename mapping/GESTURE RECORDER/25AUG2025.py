...
while cap.isOpened():
    ...
    if hands:
        for hand in hands:
            ...
            if hand_label == "left":
                ...
                client.send_message("/RHXY", [right_index_pos.x, right_index_pos.y]) 
                # Right hand logic with frequency and volume mapping
                if distance < 0.1:
                    play_start_time = time.time()
                    
                    # Frequency mapping from original script 1
                    freq = right_index_pos.x
                    min_val = -2.0
                    max_val = 7
                    mapped_freq = min_val + (max_val - min_val) * freq

                    # Volume mapping from original script 1
                    volume = right_index_pos.y
                    min_c = 1000
                    max_c = 7000
                    mapped_volume = min_c + (max_c - min_c) * volume

                    # Send frequency and volume messages
    ...
    # Send trigger for left hand tap
    if left_hand_tapped:
        trig_val = 1
